...
def index():
    if request.method == 'POST':
        ...
        if session.get('username') in usernames:
            ...
            event = Event.query.filter(Event.quantity > 0).all()  # Don't get sold out events
            ticket = Ticket.query.all()
            
            ticket = [t for t in ticket if t.bought == True]
            # notnull = []
            # for t in ticket:
            #     if t.bought == 1:
            #         notnull.append
            ...
            return render_template('event/home.html', upcoming=upcoming, myevents=myevents, tickets=ticket, user=user)
        # Redirect to login
        else:
            return """
...
def sell():
    ...
    if form.validate_on_submit():
        ...
        # No image
            ...
            error = "Upload an image for the event"
        ...
        try: 
            ...
            return render_template('event/sell.html', form=form, error=error)
        ...
        try: 
            ...
            am_pm_1 = form.starttime.data[-2:].upper() 
            am_pm_2 = form.starttime.data[-2:].upper()
            start_time = form.starttime.data[:-2]
            end_time = form.endtime.data[:-2]
            valid_start_time = start_time + " " + am_pm_1
            valid_end_time = end_time + " " + am_pm_2
            print (valid_start_time)
            print (valid_end_time)
            time.strptime(valid_start_time, "%I:%M %p")
            time.strptime(valid_end_time, "%I:%M %p")
        except:
            error = "Invalid times, please try again"
            return render_template('event/sell.html', form=form, error=error)
            
        if not isinstance(form.quantity.data, int):
            error = "Invalid input under quantity, please try again"
            return render_template('event/sell.html', form=form, error=error)
        event = Event(
            form.name.data,
            form.genre.data,
...
def confirmation(event_id):
    ...
    if request.method == 'GET':
        # If ticket already exists and not a placeholder
        emptyticket = Ticket.query.filter_by(buyer_id=user.id, event_id=event.event_id, bought=False).first()  # in case sorting needed: .order_by(Ticket.timestamp.desc())
        print('Query for user {} ticket {}'.format(user.username, emptyticket))
        # No existing empty ticket in DB, create one
        if emptyticket is None:
    ...
    if request.method == 'POST':
        ...
        Ticket.query.filter_by(buyer_id=user.id, event_id=event.event_id, bought=False).delete()
        ticket = Ticket(
            event.event_id,
            event.user_id,
            user.id,
...
def delete(event_id):
    ...
    event.name = "(Event deleted by owner!) {}".format(event.name)
    event.quantity = -1
    db.session.commit()
...
def parsename(fullname):
    ...
    if len(fullname) > 1:
        name.update(dict(lastname=''.join(fullname[1:])))
    return name